...
def get_one_hot_encoder():
    ...
    if df_join.empty:
        print("No valid data to encode.")
        # Return a default encoder and empty DataFrame
        return OneHotEncoder(), pd.DataFrame(columns=['id', 'vector'])
    
    # Prepare for OneHotEncoding
    cols = ['new_ing']
    enc = OneHotEncoder()
    ...
    try:
        ...
        # Return a default encoder and empty DataFrame on error
        return enc, pd.DataFrame(columns=['id', 'vector'])
